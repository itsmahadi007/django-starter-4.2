...
from apps.authentication.utils.verification_process import email_otp_process_before_sent, phone_otp_process_before_sent, \
    email_otp_verification, phone_otp_verification
...
def request_email_verification(request):
    ...
    using_for = request.data.get("user_for", None)

    # Check if user_for is in VerificationForStatus
    if using_for not in VerificationForStatus.values:
        return Response({"error": "Invalid using_for value"}, status=status.HTTP_400_BAD_REQUEST, )
...
def verify_email_otp(request):
    ...
    if using_for == VerificationForStatus.PASSWORD_RESET:
        if not password:
            return Response({"error": "Please provide the password"}, status=status.HTTP_400_BAD_REQUEST, )
    ...
    message, response_status = email_otp_verification(user=user, otp=otp, using_for=using_for, password=password,
                                                      request=request, two_factor=two_factor_status)
...
def request_phone_verification(request):
    ...
    # Check if user_for is in VerificationForStatus
    if using_for not in VerificationForStatus.values:
        return Response({"error": "Invalid user_for value"}, status=status.HTTP_400_BAD_REQUEST, )

    if not phone_number:
        return Response({"error": "Please provide the phone number"}, status=status.HTTP_400_BAD_REQUEST, )
    ...
    message = phone_otp_process_before_sent(user=user, phone_number=phone_number, using_for=using_for)
    return Response({"message": message})
...
def verify_phone_otp(request):
    ...
    message, response_status = phone_otp_verification(user=user, otp=otp, using_for=using_for, password=password,
                                                      request=request, two_factor=two_factor_status)
